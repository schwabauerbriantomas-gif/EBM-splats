...
class TrainingLogger:
    ...
    def _aggregate_splat_stats(self, epoch):
        all_stats = [b['splat_stats'] for b in self.log_data['current_epoch']['batches']]
        if not all_stats:
            return {}
        
        last = all_stats[-1]
        return {
            'total_splats': last.get('n_active', 0),
            'avg_frequency': last.get('mean_frequency', 0.0),
            'avg_kappa': last.get('mean_kappa', 0.0),
            'splats_added': last.get('splats_added', 0),
        }
